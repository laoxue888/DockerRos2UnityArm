...
public class IK_toolkit : MonoBehaviour
{
    ...
    // UR16e robot arm Denavit-Hartenberg parameters matrix 定义UR16e机器人手臂的Denavit-Hartenberg参数矩阵
    public static double[,] DH_matrix_UR16e = new double[7, 3] { //第一列：a (连杆长度)，第二列：alpha (连杆扭角)，第三列：d (连杆偏移)
        { 0, 0, 0.333 },
        { 0, -Mathf.PI / 2.0, 0 },
        { 0, Mathf.PI / 2.0, 0.316 },
        { 0.0825, Mathf.PI / 2.0, 0 },
        { -0.0825, -Mathf.PI / 2.0, 0.384 },
        { 0, Mathf.PI / 2.0, 0 },
        { 0.088, Mathf.PI / 2.0, 0},
    };

    // Update is called once per frame
    void Update()
    {
        // Calculate the transformation matrix for the IK 包含坐标的位姿，不是转化矩阵
        Matrix4x4 transform_matrix = GetTransformMatrix(ik); //获取末端执行器的变换矩阵

        // Reflect the matrix along the Y-axis 为了符合右手定则，反转Y轴方向
        Matrix4x4 mt = Matrix4x4.identity; //单元矩阵
        mt.m11 = -1; //第二行第二列元素的值为-1
        Matrix4x4 mt_inverse = mt.inverse;
        Matrix4x4 result = mt * transform_matrix * mt_inverse; //对transform_matrix的Y轴反向

        // Compute the inverse kinematics solutions 计算逆运动学解
        double[,] solutions = Inverse_kinematic_solutions(result);
        IK_Solutions.Clear();
        IK_Solutions = DisplaySolutions(solutions);

        // Set the robot arm joints based on the selected solution 设置机器人手臂关节
    ...
    {
        //初始化6行8列的二维数组theta
        double[,] theta = new double[7, 8];

        Vector4 P05 = transform_matrix_unity * new Vector4()
        {
            x = 0,
            y = 0,
            z = -(float)DH_matrix_UR16e[5, 2], //末端执行器的z轴坐标
            w = 1
        };
        //计算倒数的x轴坐标
