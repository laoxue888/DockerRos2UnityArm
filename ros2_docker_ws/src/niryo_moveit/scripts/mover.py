...
from niryo_moveit.msg import RobotState
...
class NiryoMoveItServer(Node):
    def __init__(self):
        super().__init__('niryo_moveit_server')
        self.srv = self.create_service(
            MoverService, 
            'niryo_moveit', 
            self.plan_pick_and_place)
...
def main(args=None):
    ...
    moveit_commander.roscpp_initialize(sys.argv)  # 注意：MoveIt的ROS2初始化可能需要不同方式
    
    niryo_moveit_server = NiryoMoveItServer()
    rclpy.spin(niryo_moveit_server)
    
    niryo_moveit_server.destroy_node()
    rclpy.shutdown()
